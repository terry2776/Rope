...
# Funzione per calcolare l'hash di un file locale (SHA-256)
...
def save_url_path_list_to_file(url_path_list, file_name):
    with open(file_name, 'w') as file:
        for item in url_path_list:
            # Stampa l'elemento in formato leggibile con ritorno a capo
            file.write(f"{item}\n")
...
# Aggiorna la lista aggiungendo l'hash calcolato per ogni file esistente
for item in url_path_list:
    path = item["path"]
    if os.path.exists(path):
        item["expected_hash"] = calculate_file_hash(path)
    else:
        item["expected_hash"] = None  # Se il file non esiste, imposta l'hash come None

# Salva la lista su un file
save_url_path_list_to_file(url_path_list, "output.txt")
print("Models list saved into output.txt file") 